  ...
  describe("CI/CD", () => {
    test("REQ.NF.239a3 — DEBE autenticar el CI con una GitHub App de la organización", async () => {
      for (const path of ENV_WORKFLOWS) {
        ...
        expect(workflow, path).toMatch(/owner:\s*\$\{\{\s*github\.repository_owner\s*\}\}/);
      }
    });

    test("REQ.NF.4bc46 — DEBE declarar el Client ID en `vars.SEMANTYK_ID`", async () => {
      for (const path of ENV_WORKFLOWS) {
        const workflow = await workspace.readText(path);
        expect(workflow, path).toContain("client-id: ${{ vars.SEMANTYK_ID }}");
      }
    });

    test("REQ.NF.d966f — DEBE declarar la private key en `secrets.SEMANTYK_PK`", async () => {
      for (const path of ENV_WORKFLOWS) {
        const workflow = await workspace.readText(path);
        expect(workflow, path).toContain(
          "private-key: ${{ secrets.SEMANTYK_PK }}",
        );
      }
    });

    test("REQ.NF.80260 — DEBE emitir un installation token en cada workflow de entorno", async () => {
      for (const path of ENV_WORKFLOWS) {
        const workflow = await workspace.readText(path);
        expect(workflow, path).toMatch(/\bid:\s*app-token\b/);
        expect(workflow, path).toMatch(/Generate App Token/);
      }
    });

    test("REQ.NF.59a15 — DEBE inyectar el token de la App como `GH_TOKEN` en las pruebas", async () => {
      for (const path of ENV_WORKFLOWS) {
        const workflow = await workspace.readText(path);
        expect(workflow, path).toContain("bun run test");
        expect(workflow, path).toContain(
    ...
    test("REQ.NF.d35b0 — NO DEBE autenticar el CI con un PAT de usuario", async () => {
      for (const path of ENV_WORKFLOWS) {
        const workflow = await workspace.readText(path);
        expect(workflow, path).not.toMatch(
          /secrets\.(?:GH_PAT|GITHUB_PAT|PAT|PERSONAL_ACCESS_TOKEN)\b/i,
        );
        expect(workflow, path).not.toMatch(
          /GH_TOKEN:\s*\$\{\{\s*secrets\./,
        );
      }
    });
  });
